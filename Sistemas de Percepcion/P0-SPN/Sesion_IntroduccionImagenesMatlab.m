...
% EJERCICIO IMAGEN COMPLEMENTARIA
I = imread('X.jpg'); % Imagen en escala de grises - ver variable en workspace
Ic = imread('P1_1.jpg'); % Imagen en color

Icom = 255-I;
imshow(I), figure, imshow(Icom)

Iccom = 255-Ic; % Haciendolo de manera directa
imshow(Ic), figure, imshow(Iccom)

Iccom = uint8(zeros(size(Ic))); % Haciendo con cada imagen por separado
R = Ic(:,:,1);
G = Ic(:,:,2);
B = Ic(:,:,3);
